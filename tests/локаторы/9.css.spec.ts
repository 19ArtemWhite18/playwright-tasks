...
test.describe('Продвинутые CSS-селекторы', () => {
  ...
  test('Комбинированные условия поиска', async ({ page }) => {
    ...
    //    - Не disabled
    const submitButton = page.locator('form > button.btn.submit-btn'); // TODO(student): замените на корректный локатор
...
test.describe('Динамический контент с условиями', () => {
  ...
  test('Фильтрация динамических элементов', async ({ page }) => {
    ...
    const dynamicButton = page.locator('.disabled:has-text("Недоступно"):not([type="submit"])'); // TODO(student): замените на корректный локатор
