...
test('1. Проверка атрибутов основной кнопки', async ({ page }) => {
  ...
  const toSend = page.getByRole('button', { name: 'Отправить' });

  await expect(toSend).toHaveAttribute('data-action', 'submit');
  await expect(toSend).toHaveAttribute('title', 'Основная кнопка');

  await page.getByRole('button', { name: 'Переключить атрибуты' }).click();

  await expect(toSend).toHaveAttribute('data-action', 'cancel');
  await expect(toSend).toHaveAttribute('title', 'Отмена действия');
});
...
test('2. Проверка отключения кнопки', async ({ page }) => {
  ...
  await toSend.click();
  await expect(toSend).not.toHaveAttribute('disabled');

  await page.getByRole('button', { name: 'Отключить кнопку' }).click();
  await expect(toSend).toHaveAttribute('disabled', '');
...
test('3. Проверка атрибутов изображения', async ({ page }) => {
  ...
  const imagine = page.getByAltText('Аватар пользователя');

  await expect(imagine).toHaveAttribute('src', 'user.jpg');
  await expect(imagine).toHaveAttribute('alt', 'Аватар пользователя');
  await expect(imagine).toHaveAttribute('width', '200');
});
...
test('5. Проверка data-атрибутов', async ({ page }) => {
  ...
  await expect(container).toHaveAttribute('data-user-id');
