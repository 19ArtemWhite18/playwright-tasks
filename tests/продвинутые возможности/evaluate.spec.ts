...
test.describe('Практика работы с page.evaluate()', () => {
  ...
  test('Получение текстового содержимого элемента', async ({ page }) => {
    ...
    const counterValue = await page.evaluate(() => document.getElementById('counter')?.textContent);
    ...
    const updatedValue = await page.evaluate(() => document.getElementById('counter')?.textContent);
  ...
  test('Модификация DOM через evaluate', async ({ page }) => {
    ...
    await page
      ...
      .evaluate(
        (element) =>
          (element.innerHTML = '<h3>Новое содержимое</h3><p>Сгенерировано в evaluate()</p>'),
      );
